# This Makefile is for the Apache::Yaalr extension to perl.
#
...
#     ABSTRACT_FROM => q[lib/Apache/Yaalr.pm]
#     AUTHOR => q[Jeremiah Foster <jeremiah@jeremiahfoster.com>]
#     NAME => q[Apache::Yaalr]
#     PREREQ_PM => { Template=>q[0] }
#     VERSION_FROM => q[lib/Apache/Yaalr.pm]
...
FULLEXT = Apache/Yaalr
BASEEXT = Yaalr
PARENT_NAME = Apache
DLBASE = $(BASEEXT)
VERSION_FROM = lib/Apache/Yaalr.pm
OBJECT = 
LDFROM = $(OBJECT)
LINKTYPE = dynamic
BOOTDEP = 
...
TO_INST_PM = lib/Apache/Yaalr.pm

PM_TO_BLIB = lib/Apache/Yaalr.pm \
...
DISTNAME = Apache-Yaalr
DISTVNAME = Apache-Yaalr-0.02.8
...
manifypods : pure_all  \
	...
	$(NOECHO) $(POD2MAN) --section=$(MAN3EXT) --perm_rw=$(PERM_RW) \
	  lib/Apache/Yaalr.pm $(INST_MAN3DIR)/Apache::Yaalr.$(MAN3EXT) 
...
metafile : create_distdir
	...
	$(NOECHO) $(ECHO) 'name:         Apache-Yaalr' >> META_new.yml
	$(NOECHO) $(ECHO) 'version:      0.02.8' >> META_new.yml
	$(NOECHO) $(ECHO) 'version_from: lib/Apache/Yaalr.pm' >> META_new.yml
...
ppd:
	$(NOECHO) $(ECHO) '<SOFTPKG NAME="$(DISTNAME)" VERSION="0,02,8,0">' > $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '    <TITLE>$(DISTNAME)</TITLE>' >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '    <ABSTRACT>Yet Another Apache Log Reader</ABSTRACT>' >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '    <AUTHOR>Jeremiah Foster &lt;jeremiah@jeremiahfoster.com&gt;</AUTHOR>' >> $(DISTNAME).ppd
...
pm_to_blib : $(TO_INST_PM)
	$(NOECHO) $(ABSPERLRUN) -MExtUtils::Install -e 'pm_to_blib({@ARGV}, '\''$(INST_LIB)/auto'\'', '\''$(PM_FILTER)'\'')' \
	  lib/Apache/Yaalr.pm blib/lib/Apache/Yaalr.pm 
